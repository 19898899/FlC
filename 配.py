...
def generate_clash_config(nodes_data, token):
    """
    生成Clash配置文件（与你提供的完全一样格式）
    """
    # 基础配置
...
"""
    ...
    # 1. 生成proxies部分（紧凑格式）
    proxies_lines = []
    for node in nodes_data.get('goserverlist', []):
        name = node.get('name', '').replace('"', '\\"')  # 转义引号
        host = node.get('host', '')
        port = node.get('remotePort', 0)
        method = node.get('method', 'chacha20')
        password = node.get('password', '')
        protocol = node.get('protocol', 'auth_chain_a')
        obfs = node.get('obfs', 'tls1.2_ticket_auth')
        protocol_param = node.get('protocol_param', '75025855:O0rwJP')
        obfs_param = node.get('obfs_param', '')
        
        proxy_line = f'  - {{name: "{name}", server: {host}, port: {port}, type: ssr, cipher: {method}, password: {password}, protocol: {protocol}, obfs: {obfs}, protocol-param: {protocol_param}, obfs-param: "{obfs_param}"}}'
        proxies_lines.append(proxy_line)
    
    proxies_section = '\n'.join(proxies_lines)
    
    # 2. 生成代理组部分
    proxy_names = [node.get('name', '') for node in nodes_data.get('goserverlist', [])]
    
    # 节点选择组（所有节点）
    select_proxies = ['♻️ 自动选择', '🎯 全球直连'] + proxy_names
    select_section = '      - ' + '\n      - '.join(select_proxies)
    
    # 自动选择组（所有节点）
    auto_proxies = proxy_names
    auto_section = '      - ' + '\n      - '.join(auto_proxies)
    
    # 其他组的代理列表
    other_proxies = ['🔰 节点选择', '♻️ 自动选择', '🎯 全球直连'] + proxy_names
    other_section = '      - ' + '\n      - '.join(other_proxies)
...
Push the commit or tag
  ...
  remote: Permission to 19898899/FlC.git denied to github-actions[bot].
  fatal: unable to access 'https://github.com/19898899/FlC.git/': The requested URL returned error: 403
...
"""
    ...
    # 4. 组合配置
    config_yaml = clash_config.format(proxies_section, proxy_groups, rules)
    
    return config_yaml  # 返回字符串 
